    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.is_container and not insect.is_container and not self.ant.contained_ant:
                    self.ant.contain_ant(insect)
                elif insect.is_container and not self.ant.is_container and not insect.contained_ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        limit = 0
        while search is not hive and limit <= self.max_range:
            # print('Searching place', search)
            if search.bees:
                # print('It has bees. Returning a random bee')
                return random_or_none(search.bees)
            # print('It has no bees. Going to place', search.entrance)
            limit += 1
            search = search.entrance
        return None
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self, armor=1):
        ...
        if not QueenAnt.already_exists:
            self.true_queen = QueenAnt.already_exists = True
            self.doubled_ants = []
        Ant.__init__(self, armor)
    ...
    def action(self, colony):
        ...
        if not self.true_queen:
            ...
            return
        double_place = self.place.exit
        while double_place:
            ant_in_place = double_place.ant
            if ant_in_place:
                if ant_in_place not in self.doubled_ants:
                    ant_in_place.damage *= 2
                    self.doubled_ants.append(ant_in_place)
                if ant_in_place.is_container and ant_in_place.contained_ant:
                    if ant_in_place.contained_ant not in self.doubled_ants:
                        ant_in_place.contained_ant.damage *= 2
                        self.doubled_ants.append(ant_in_place.contained_ant)
            double_place = double_place.exit
...
def apply_effect(effect, bee, duration):
    ...
    action_after_effect = effect(bee.action, bee)
    def second_action(colony):
        nonlocal duration
        if duration > 0:
            action_after_effect(colony)
        else:
            original_action(colony)

        duration -= 1
    bee.action = second_action
